...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default = "/lustre/BIF/nobackup/silva045/ocr/images/test", type=str)
...
def infer(images_path, model, postprocessors, device, output_path):
    ...
    for img_sample in images_path:
        ...
        if ncols == 0:
            ...
            fig, axs = plt.subplots(ncols=ncols, nrows=1, figsize=(18, 6))
            if ncols == 1:
                axs = [axs]  # Ensure axs is always a list

            for idx, ax_i, (xmin, ymin, xmax, ymax) in zip(keep.nonzero(), axs, boxes[0:3]):
                ax_i.imshow(dec_attn_weights[0, idx].view(h, w), interpolation='nearest', extent=extent)
                ax_i.axis('off')
                ax_i.set_title(f'query id: {idx.item()}')
                ax_i.imshow(im, alpha=.35, interpolation='bilinear', extent=extent)
                ax_i.set_title(CLASSES[probas[idx].argmax()])
