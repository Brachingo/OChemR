...
def runExperiment(loc, labelled_images):
    ...
    b_sizes = params[
        "molecules_sizes"
    ]  # [30,45,60] # Size of chemical bonds in molecules.
    rotations = params["molecules_rotations"]  # [0,30,330]
    n_molecules = params[
        "num_molecules_per_reaction"
    ]  # n molecules that will appear in each final image.
    n_samples = params[
        "num_reactions_per_epoch"
    ]  # number of samples created per each epoch (with its variables, rotation...)
    epochs = params["epochs"]  # num of epochs

    # Print metadata
    print(
        f" @author: Mark Martori Lopez - IBM Research \n\n---- Syntetic dataset creation with the following params: ----"
    )
    ...
    for epoch in range(epochs):
        ...
        for it in range(0, n_samples):
            ...
            if it % 250 == 0 and it != 0:
                time_now = time.time()
                print(
                    f"Image {it}/{n_samples} of epoch {epoch} completed.",
                    end="\n",
                    flush=True,
                )
                print(
                    f"{round(((total_it + it) / (n_samples * epochs)) * 100, 2)}% of total task done in {round(abs(time_now - initial_time))} seconds."
                )
        total_it += n_samples
        tf = time.time()
        print(
            f"Epoch {epoch + 1}/{epochs} completed in {round(abs(tf - initial_time))} seconds.",
            end="\n",
            flush=True,
        )
...
loc_parser.add_argument(
    "-l",
    "--log-folder",
    help="Folder to which to save logs",
    action="store",
    default="logs",
)
loc_parser.add_argument(
    "-p",
    "--params-folder",
    help="Folder that contains json file with params",
    action="store",
)
loc_parser.add_argument(
    "-cr", "--crop", help="Cropping Data Augmentation", default=(0, 0)
)
loc_parser.add_argument(
    "-ph", "--p-horiz", help="Probability of Horizontal flip", default=0
)
loc_parser.add_argument(
    "-pv", "--p-vert", help="Probability of Vertical flip", default=0
)
loc_parser.add_argument(
    "-pbr", "--p-bright", help="Probability of brightness change", default=0
)
loc_parser.add_argument(
    "-b", "--blurred", help="add blurred Data Augmentation", default=False
)
loc_parser.add_argument(
    "-sk", "--skeletonization", help="add blurred Data Augmentation", default=False
)
loc_parser.add_argument(
    "-cl", "--colored_lines", help="add coloured blobs Data Augmentation", default=False
)
loc_parser.add_argument(
    "-li",
    "--labelled-images",
...
if __name__ == "__main__":
    ...
    os.makedirs(
        loc, exist_ok=True
    )  # Use makedirs to create intermediate directories if needed
    logger.init_logger(loc)
